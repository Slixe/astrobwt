...
fn core(output: &mut [u8; BLOCK_SIZE], input: &[u8; COUNTER_SIZE], k: &[u8; KEY_SIZE], c: &[u8; CONSTANT_SIZE]) {
    ...
    let (mut x0, mut x1, mut x2, mut x3, mut x4, mut x5, mut x6, mut x7, mut x8) = (j0, j1, j2, j3, j4, j5, j6, j7, j8);
	let (mut x9, mut x10, mut x11, mut x12, mut x13, mut x14, mut x15) = (j9, j10, j11, j12, j13, j14, j15);
